...
MainShape::MainShape(const MFnMesh& fnMesh, const Arguments& args, ShapeIndex shapeIndex) : MeshShape(shapeIndex)
{
	m_skeleton = std::make_unique<MeshSkeleton>(fnMesh, args);
	...
	m_vertices = std::make_unique<MeshVertices>(*m_indices, m_skeleton.get(), fnMesh, shapeIndex, args);
}
