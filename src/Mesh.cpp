...
Mesh::Mesh(const MDagPath& dagPath) : m_dagPath(dagPath)
{
	...
	for (auto shaderIndex=0; shaderIndex<shaderUsages.size(); ++shaderIndex)
	{
		...
		{
			m_renderables[shaderIndex] = std::make_unique<MeshRenderable>(
				shaderIndex, *m_semantics, *m_vertices, *m_indices);
		}
	}
}
...
void Mesh::dump(const std::string& name, const std::string& indent) const
{
	...
	{
		MFnDependencyNode fnDepNode(shaders[pair.first]);
		pair.second->dump(fnDepNode.name().asChar(), subIndent2);
		cout << "," << endl;
	}
	cout << subIndent << "]" << endl;
	cout << indent << "}" << endl;
