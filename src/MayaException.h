...
class MayaException : public std::runtime_error {
  public:
    MayaException(const MStatus error, const std::string &message)
        : runtime_error(message), status(error) {}

    const MStatus status;

    /** Formats an error message and throws me */
    static void throwIt(const MStatus status, const std::string &message,
...
#define THROW_ON_FAILURE(__expression__)                                       \
    {                                                                          \
        ...
        if (MStatus::kSuccess != __status__)                                   \
            MayaException::throwIt(__status__, "", __FILE__, __LINE__,         \
                                   __FUNCTION__);                              \
    }
...
#define THROW_ON_FAILURE_WITH(__expression__, __message__)                     \
    {                                                                          \
        ...
        DEBUG_ASSERT_SUCCESS(__status__);                                      \
        if (MStatus::kSuccess != __status__)                                   \
            MayaException::throwIt(__status__, (__message__), __FILE__,        \
                                   __LINE__, __FUNCTION__);                    \
    }

template <typename... Args>
bool checkAndReportStatus(const MStatus &status, const char *format,
                          Args... args) {
