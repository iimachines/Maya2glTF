...
struct DagHelper
{
    ...
    static MStatus      getPlugConnectedTo(const MObject& node, const MString& attribute, MPlug& plug);

    /** Find a node connected to a node's attribute, returns kNull when not found */
    static MObject		findNodeConnectedTo(const MObject& node, const MString& attribute);

    /* Find the node connected to a plug, returns kNull when not found */
    static MObject		findNodeConnectedTo(const MPlug& plug);

    static MObject		findSourceNodeConnectedTo(const MObject& node, const MString& attribute);
    static MObject		findSourceNodeConnectedTo(const MPlug& inPlug);

    static MStatus  	getPlugValue(const MObject& node, const char* attributeName, double &value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, float &value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, bool& value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, int& value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, MColor& value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, MString& value);
    static MStatus		getPlugValue(const MObject& node, const char* attributeName, MVector& value);
    static MStatus      getPlugValue(const MObject& node, const char* attributeName, MStringArray& value);

    static MStatus		getPlugValue(const MPlug& plug, bool& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, int& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, short& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, char& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, float& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, double& value) { return plug.getValue(value); }
    static MStatus		getPlugValue(const MPlug& plug, MString& value) { return plug.getValue(value); }
    ...
    static MStatus		getPlugValue(const MPlug& plug, float& x, float& y, float& z);
    ...
    static bool			SetPlugValue(MPlug& plug, const std::string& value);
    ...
    // [PeterV] Get or create a new intermediate shape connected to the targetPlug.
    // This is used to make sure we can read valid geometry arriving at the targetPlug
    // (e.g. just the input plugs of the blend shape controller and skin pre-bind mesh)
    static MObject GetOrCreateIncomingShapeNode(MPlug& targetPlug);
    static MObject GetOrCreateOutgoingShapeNode(MPlug& sourcePlug);
    ...
    // [PeterV] Create a new message attribute (and plug), if not already present.
...
private:
    template <class ValueType>
    static MStatus getPlugValueImpl(const MObject& node, const char* attributeName, ValueType& value)
    {
        MStatus status;
