...
void MeshBlendShapeWeights::clearWeightsExceptFor(const WeightPlugEntry* fullWeightEntry) const
{
    const auto fullWeightIndex = fullWeightEntry ? fullWeightEntry->plugIndex : -1;
    ...
    for (auto& pair : m_weightPlugEntries)
    {
        ...
        MStatus status;
        MPlug weightPlug = m_weightArrayPlug.elementByLogicalIndex(plugIndex, &status);
    ...
    if (m_isDeformerModified)
    {
        ...
        for (auto& pair : m_weightPlugEntries)
        {
            ...
            if (!checkAndReportStatus(status,
                "Failed to get blend shape weight plug '%s'!", weightName))
                continue;

            if (!checkAndReportStatus(weightPlug.setDouble(entry.originalWeight),
                "Failed to restore blend shape weight '%s'!", weightName))
                continue;

            if (!checkAndReportStatus(weightPlug.setLocked(entry.isLocked),
                "Failed to restore blend shape locked state '%s'!", weightName))
                continue;
            ...
            for (auto j = 0U; j < srcConnections.length(); j++)
            {
                if (!checkAndReportStatus(dagModifier.connect(weightPlug, srcConnections[j]),
                    "Failed to restore blend shape weight source connection '%s'!", weightName))
                    continue;
            ...
            for (auto j = 0U; j < dstConnections.length(); j++)
            {
                if (!checkAndReportStatus(dagModifier.connect(dstConnections[j], weightPlug),
                    "Failed to restore blend shape weight destination connection '%s'!", weightName))
                    continue;
